  ...
  return (
    <div className="mt-10">
      <p className="text-[10px] uppercase tracking-[0.2em] font-bold text-zinc-600 mb-3 flex items-center gap-2">
        <span className="text-[#C8AA6E]">★</span> Favorites
      </p>
      <div className="flex flex-wrap gap-2">
        {favorites.map((f) => (
          <div
            key={`${f.name}#${f.tag}`}
            className="group flex items-center gap-2 bg-white/[0.03] border border-white/[0.07] hover:border-[#C8AA6E]/20 hover:bg-white/[0.05] rounded-lg pl-3 pr-1 py-1.5 transition-all cursor-pointer"
            onClick={() =>
              router.push(
